...
class GCS {
  ...
  setCred(params) {
    if (
      !(
        typeof params.clientId === "string" || params.clientId instanceof String
      )
    )
      throw new Error(
    ...
    if (!this.toolKit.detectURL(params.redirectUrl))
      throw new Error(
        "params.redirectUrl is invalid value. You must set string value which is an URL."
      );
  ...
  uploadFiles(fileList, bucketName, callback, accessToken = null) {
    ...
    if (
      ...
      throw Error("fileList type is invalid. It must be FileList.");
  ...
  async uploadFile(event) {
    ...
    if (!this.file) {
      ...
      const requestURL = `https://storage.googleapis.com/upload/storage/v1/b/${this.bucketName}/o`;
      const queryString = `?uploadType=media&name=${this.file.name}`;
